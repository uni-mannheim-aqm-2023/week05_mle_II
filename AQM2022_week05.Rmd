...
*What can impact the curvature, apart from the number of observations, which we have seen last time?*
...
Hessian is simply a matrix of second derivatives of the log-likelihood function with respect to parameters (or the derivatives of the Score, or the rate of change of the rate of change). 

*What is in this formula and how is related to the curvature of the curve?*

Remember how a linear model is basically a quadratic function? Let's look at two example quadratic functions we plotted above:

$$
f_1(x) = -0.3x^2 \, \text{and} \, f_2(x) = -3x^2
$$
These would be the first derivatives, the indicators of the instantaneous rate of change in $f(x)$ at some value $x_0$.

$$
f_1'(x) = 2 \cdot(-0.3x) = -0.6x \;  \text{and} \; f_2'(x) = -3x^2 = -6x
$$
And now the second derivatives, i.e. the rate of change of the rate of change. 

$$
f_1''(x) = - 0.6 \; \text{and} \; f_2''(x) = -6
$$
...
Second, the Central Limit Theorem causes the sampling distribution of $\hat{\theta}_{ML}$ to be asymptotically normal. You can think of a maximum of the (log-)likelihood as essentially the mean of a lot of likelihoods. 
Third, the variance of the sampling distribution of $\hat{\theta}_{ML}$ is closely related to the matrix of second derivatives, the Hessian.
...
$$
\hat{SE}(\hat\theta) = \sqrt{\Bigg(-\underbrace{\frac{\partial^2 \ell(\theta|y)}{\partial \theta \partial \theta'}}_{\text{Hessian}}\Bigg)^{-1}}
$$

```{r Exercise I: From the Hessian to Standard Errors, eval=FALSE, collapse=TRUE}
# get the Hessian from optim
res$hessian
# Calculate the observed Fisher Matrix
-res$hessian
...
mle_bs <- function(data) {
  ...
  bs_samp <- data[sample(1:nrow(data), replace = TRUE),] # sample(data, replace = T)
  
  # 2) extract y and X from the data frame
  y <- bs_samp$y
  X <- cbind(1, bs_samp$x1, bs_samp$x2)
...
You will have two weeks for homework assignment 3. In the homework you will have another look at maximum likelihood by hand (which you can already do) and implement heteroskedastic regression (Thomas will tell you more about this in next week's lecture).
